...
func main() {
	rootDir := "temp/taxi-sha256" // Change to where to unzipped taxi-sha256.zip
	...
	for name, signature := range sigs {
		go signatureWorker(rootDir, name, signature, ch)
	}
...
func signatureWorker(rootDir, name, signature string, ch chan bool) {
	ok := true
...
// Series:   processed 10 files in 5.348466959s
// Parallel: processed 10 files in 1.165307458s
